...
- **What the platform does today (WP-15), and where it stops short:** the return carries the failing job **names** rather than the error block, because reading the log means `GitProviderPort.getJobLog`, whose redaction obligation for a run-scoped credential is still open — see **Q55** in `docs/OPEN-QUESTIONS.md`; closing it is what makes the error block safe to put in a return reason that is stored on the task and handed to the next Implementation run. The tamper check, the reproduction gate and flaky detection are not implemented either: the gate is pass/fail on the pipeline's status plus the failing job names, and everything else in this section is still the specification.
