...
| Discovery (onboarding, Step 2) | ✔ (repository) | – | read-only cmds (`ls`, `cat`, `grep`, `rg`, `find`, `git log|diff|show|blame|status`) + the project's declared commands and lockfile installs (WP-54) | – | – | – | – | draft pages as `bootstrap` proposals, never auto-applied | – (questions go into the draft) |
| Ask (ask-the-task, WP-31) | ✔ (the task's own context as data blocks: ticket, artifacts, runs, human actions — no checkout: the run has a container with the `kb` skill and no clone (WP-74)) | – | – | – | – | – | `get_task_context`, `kb_search` (read-only) | – | – (an ask is answered from the audit trail; questions to a human are not its job) |
...
*Amended at WP-54 (session 8): the gap the note above records is closed by the second way out — Q69 answer (ii), BD-025's WP-54 amendment. Each role has a shipped baseline the project narrows (`COMMAND_BASELINE_BY_ROLE`): the Reviewer, the Acceptance Tester and Discovery run the read-only list plus the lockfile installs and the project's declared commands, the Developer adds the git write verbs, and the Investigator and the Architect hold a shell on the read-only list (plus the read verbs of an observability skill their project is bound to). The code follows this table's Shell column for every role, enforced by a test that enumerates the roles. R1, R2 and R6 are detected as product/17 words them, with one residual: a devcontainer or compose target is read, not run, because `docker *` is blocked for every stage.*
