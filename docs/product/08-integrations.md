...
|---|---|---|---|---|
| **Task management** | Pick-up rules (label / status / epic / query), read ticket (title, description, comments, attachments text, links, type, priority), read issue links (blocked-by) and the parent epic with sibling tickets, map pipeline states to statuses, maintain the workpad comment, ask/answer questions, set labels/fields, link MR, create follow-up tickets, lint new tickets (opt-in) | `ticket.matched`, `ticket.updated`, `ticket.comment.added`, `ticket.status.changed` | `transition`, `comment`, `label`, `link`, `assign` | read ticket, add comment (via platform tool, not raw credentials) |
| **Git provider** | Clone/push with a bot identity, branches, open/update MR (draft/ready), MR discussions, reviewers (incl. CODEOWNERS lookup), CI status, job logs and coverage artifacts, diff stats, rebase/conflict detection, merge/close events, review-only mode on human MRs, history export for bootstrap | `mr.opened`, `mr.updated`, `mr.review.comment`, `mr.approved`, `mr.merged`, `mr.closed`, `ci.pipeline.finished` | `open_mr`, `update_mr`, `comment`, `resolve_thread`, `set_reviewers`, `get_ci_logs` | CLI (`glab`/`gh`) with a project-scoped token in a skill |
| **Communication** | Notify (task picked up, question, ready for merge, escalation, budget), threaded Q&A with buttons and free text, daily digest with quiet hours, ask-the-task threads | `message.reply`, `action.clicked` | `post`, `post_question`, `update`, `digest` | none directly; `ask_human` / `notify_human` platform tools |
