...
## Stack
React 19 + Vite 8 + TypeScript; TanStack Router (SPA, file-based routes, zod search params) + TanStack Query; shadcn/ui on `@base-ui/react` + Tailwind 4; TanStack Table + Virtual (chat mode for transcripts); cmdk, sonner, react-hook-form + zod, react-hotkeys-hook, react-resizable-panels; CodeMirror 6 (+ yaml, markdown, lint, merge, json-schema); streamdown for live assistant markdown; react-markdown + rehype-sanitize + shiki (worker) for KB/artifacts; `@pierre/diffs` behind one `<Diff>` component (lazy-loaded); anser for ANSI; Recharts 3. Served as a static bundle by the app process with SPA fallback; same origin as the API and SSE — `apps/server/src/web/` since WP-15j, where the fallback is **allow-list shaped**: `index.html` is returned for a path whose first segment the route tree below declares and for nothing else, so `/api/*`, `/events`, `/webhooks/*` and the probes can never be answered with the shell, and a missing asset is a 404 rather than HTML. Hashed assets under `assets/` are immutable for a year; the shell itself is `no-cache`, because a shell a browser kept would pin a bundle a deploy deleted. `APP_WEB_ROOT` overrides the directory (technical/12); an absent bundle is named in a warning and leaves the API serving. The bundle is **coded on the wire** — `br` preferred, `gzip` otherwise, nothing below 1 024 bytes and nothing already compressed, each file coded once per coding and cached for the life of the process (`apps/server/src/web/encoding.ts`) — which is what makes TD-013:8's *gzipped* budget a statement about the bytes an instance ships rather than about a number only `pnpm bundle:check` ever sees; the API half is TD-002's `@fastify/compress`, with `text/event-stream` and `/api/auth/*` excluded. Every response the bundle answers carries `x-frame-options: DENY` and a **Content-Security-Policy measured on the built bundle** (`apps/server/src/web/csp.ts`): `default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self'; font-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'`. No `'unsafe-inline'` and no nonce: the shell has **no** inline script (the theme is applied in `main.tsx`, `apps/web/index.html` says so on purpose), Tailwind emits one stylesheet at build time, and React applies a `style` prop through CSSOM, which `style-src` does not govern. What it excludes is stated where it is decided — `require-trusted-types-for` (react-dom writes `innerHTML` itself), `worker-src` beyond `'self'` (this bundle has no worker), and a library that writes a `style` attribute or injects a `<style>` element, which would be refused loudly rather than silently allowed. The proof that the application runs under it is a browser: the Playwright harness serves the built bundle under this exact header, `test/web-e2e/csp.spec.ts` fails on a single `securitypolicyviolation`, and a probe pair shows the policy is being enforced. zod's JIT compilation is turned off in the browser entry for the same reason (`apps/web/src/zod-jitless.ts`): its `Function("")` probe is refused by `script-src` and reported every page load, and a violation channel that is never empty is one nobody reads.
