...
> Not started. This file lists the constraints inherited from the product decisions and the candidate choices to evaluate, so Round 2 starts from a checklist rather than a blank page. Nothing here is decided; decisions will be `decisions/technical/TD-nnn`.
