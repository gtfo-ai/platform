...
**Session 5 so far.** `main` is at **`23c5d37`** (WP-21: `ci` `34742367222` and `image` `34742367313`
...
Q69) — plus three ci-fixes for harness flakes, an architect ruling (TD-026), TD-018 amended as built,
product/13 amended twice, backlog entries 30–49, Q63–Q69, and a refinement to rule 61. **M1 has three
rows left**: **WP-15i** (the eleven task and run commands; brief drafted in the orchestrator's scratchpad,
to be re-derived from the row and the census's admitted-gap list if lost), **WP-15j** (serving the SPA;
brief drafted) and **WP-23** (the guides and the notices; brief drafted) — in that order, because the
user guide describes what the other two make true. Then M2, starting with WP-24 and WP-25 (briefs
...
| WP-15i | **The task and run command surface** — the eleven commands the SPA calls and nothing serves (backlog **29**'s write half) | WP-02, WP-06, WP-15, WP-15e, WP-15h | no | TODO | — | Filed by a refiner in session 5. `POST /api/runs/:run_id/steer` stays WP-27's; `human_actions` has no writer today; before WP-23, WP-27, WP-28 and WP-31. |
