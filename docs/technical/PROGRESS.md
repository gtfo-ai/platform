...
**Session 5 so far.** `main` is at **`e4eaf8a`** (WP-15e: `ci` `34734001501` and `image` `34734001547`
...
**Nine work packages closed in session 5 so far** — backlog 28 (the e2e teardown flake, now standing
...
18's class: a version column and the column partition, all 21 sites covered) — plus three ci-fixes for
harness flakes (three distinct mechanisms, all found by CI on docs-only commits), an architect ruling
(TD-026), TD-018 amended as built, backlog entries 30–43, Q63–Q67, and a refinement to rule 61 from an
implementer's admissions against itself. **Two M1 rows remain**: **WP-21** (the onboarding wizard,
discovery and readiness; brief drafted in the orchestrator's scratchpad, to be re-derived from the row and
product/06 and /17 if lost) and **WP-23** (the guides and the notices; brief drafted). Then M2, starting
with WP-24 (brief drafted). Rows the census still names as unowned: the twelve task and run **commands**
(backlog 29's remainder) and serving the SPA (backlog 33) — both should precede WP-23's user guide, and
neither has a row; the orchestrator files them before starting M2. **Both rows now exist** (refiner, session
...
(6) **WP-15h part 2 is DONE** at `d1a5116`; (7) **WP-14a is DONE** at `dcc1f21`; (8) **WP-15e is DONE** at `e4eaf8a`; (9) **WP-21** is next, then
WP-23 — with the twelve commands and the SPA serving (backlog 29's remainder and 33) to be given rows
before M2. Then **WP-15h part 2** (the seven reads the census attributes to it), WP-14a, WP-21, WP-23; then
M2 (WP-24–33); then M3.
