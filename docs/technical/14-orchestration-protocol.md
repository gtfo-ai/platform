...
repeat:
  ...
  2. Ensure clean tree and green verify on main (git status; pnpm -s verify). If red: spawn implementer "fix main" before anything else.
  ...
  4. On report: run pnpm -s verify and the WP's test target yourself. FAIL → send the failure to the same implementer (SendMessage) with "fix, then re-report" (max 2 times), then treat as review round.
...
- Keep every tool output short: use `pnpm -s verify 2>&1 | tail -40`, `git log --oneline -5`, never `cat` sources.
...
`pnpm -s verify` = lint + typecheck + unit + contract tests; `pnpm -s verify:integration` = Testcontainers suites; `pnpm -s verify:e2e` = fake-Claude compose e2e; `pnpm -s verify:ui` = web tests. Each target prints a one-line PASS/FAIL summary at the end.

## Definition of done per WP
Acceptance criteria from the plan met; tests per `10-testing-strategy.md`; `pnpm -s verify` green; reviewer APPROVE; ledger updated; commit pushed; no new `[unverified]` claims in docs without a TODO entry; `.env.example` and `CLAUDE.md` updated when relevant.
