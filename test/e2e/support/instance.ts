...
export const startInstance = async (options: StartInstanceOptions = {}): Promise<Instance> => {
  ...
  const env: Record<string, string | undefined> = {
    ...
    TZ: 'UTC',
    // The floor `requiredPoolConnections` computes for `ROLE=all` at concurrency 1 — 18 since
    // WP-32 added the digest tick to WP-21's `onboarding.discovery`, WP-18b's three and WP-18a's
