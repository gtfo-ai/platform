...
export const startInstance = async (options: StartInstanceOptions = {}): Promise<Instance> => {
  ...
  const env: Record<string, string | undefined> = {
    ...
    TZ: 'UTC',
    // The floor `requiredPoolConnections` computes for `ROLE=all` at concurrency 1 — 17 since
    // WP-21 added `onboarding.discovery` to WP-18b's three and WP-18a's index worker. A value below
    // it is refused at boot, which is how this line keeps finding out.
    APP_DB_POOL_MAX: '17',
