...
export const startInstance = async (options: StartInstanceOptions = {}): Promise<Instance> => {
  ...
  const env: Record<string, string | undefined> = {
    ...
    TZ: 'UTC',
    // The floor `requiredPoolConnections` computes for `ROLE=all` at concurrency 1 — 19 since
    // WP-31 added the `task.ask` worker to WP-32's digest tick, WP-21's `onboarding.discovery`,
    // WP-18b's three and WP-18a's index worker. A value below it is refused at boot, which is how
    // this line keeps finding out.
    APP_DB_POOL_MAX: '19',
