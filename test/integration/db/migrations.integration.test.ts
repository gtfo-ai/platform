...
const EXPECTED_TABLES = [
  ...
  'shadow_batches',
  'shadow_reports',
  // WP-41, migration 0034: the two facts whose only record was an event — when a task's merge
  // request merged, and the daily counters of the four metric events nothing read. Everything else
...
describe('migrate on an empty PostgreSQL 18', () => {
  ...
  it('records the storage policy of every table that declares one', async () => {
    ...
    expect(policy).toEqual([
      ...
      // WP-48 (migration 0036): the mark that a curation happened, and the recovery's own columns
      // beside it. `read_write` because a row is written once by the curation, or written by the
      ...
      // an upsert that adds, and a projection that could never be corrected could not be rebuilt
      // after a defect in the fold — and registered rather than defaulted so the "registry lists
