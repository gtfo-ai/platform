 ...
 * executes.
 *
 * The third place the path could drift is `compose.yml`, which declares the variable an operator
 * overrides it with. It deliberately carries **no copy of the path** — an empty default means "the
 * directory the image carries" — and this file allows either shape while refusing a *different*
 * path, so tightening compose later is a change to compose rather than a change to this test.
 */
...
describe('the bundle directory the image writes and the one the server reads', () => {
  ...
  it('are named by compose without a second copy of the path', () => {
    // TD-020: a variable the server reads is a variable the operator can set. What compose must
    // not do is name a *different* directory — an empty default (the image's own) is the shape it
    // ships with.
